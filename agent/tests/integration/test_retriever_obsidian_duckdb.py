...
def test_get_documents_by_ids_01(
    ...
    dk_obsidian_retriever: ObsidianLlamaRetriever,
    dk_entity: type,
    ...
    # 試験準備
    doc = vault_docs[0]
    VaultDb.sync([doc], dk_cxt.get_engine(), dk_entity)

    # 試験実施
    assert doc.id is not None
    results = dk_obsidian_retriever.get_documents_by_ids([doc.id])
    ...
    # 観点2
    assert dk_obsidian_retriever.get_documents_by_ids(["nonexistent-uuid"]) == []
...
def test_get_backlinks_01(
    ...
    VaultDb.sync([doc_a, doc_b], dk_cxt.get_engine(), dk_entity)

    # 試験実施
    results = dk_obsidian_retriever.get_backlinks(link_tgt_id)
    ...
    # 観点3
    assert dk_obsidian_retriever.get_backlinks(path_to_document_id("nonexistent_target.md")) == []
...
def test_sync_chunks_01(
    ...
    # --- ステップ1: 追加 ---
    VaultDb.sync([noise_doc, target_doc], dk_cxt.get_engine(), dk_entity)
    dk_obsidian_retriever.sync_chunks()

    # 観点1
    results = dk_obsidian_retriever.get_documents_by_ids([noise_doc.id, target_doc.id])
    ...
    VaultDb.sync([noise_doc, updated_target], dk_cxt.get_engine(), dk_entity)
    dk_obsidian_retriever.sync_chunks()

    # 観点2
    target_result = dk_obsidian_retriever.get_documents_by_ids([target_doc.id])[0]
    assert target_result.page_content == updated_target.page_content
    assert target_result.metadata == updated_target.metadata
    noise_result = dk_obsidian_retriever.get_documents_by_ids([noise_doc.id])[0]
    assert noise_result.page_content == noise_doc.page_content
    assert noise_result.metadata == noise_doc.metadata

    # --- ステップ3: 削除 ---
    VaultDb.sync([noise_doc], dk_cxt.get_engine(), dk_entity)
    dk_obsidian_retriever.sync_chunks()

    # 観点3
    assert dk_obsidian_retriever.get_documents_by_ids([target_doc.id]) == []
    noise_result = dk_obsidian_retriever.get_documents_by_ids([noise_doc.id])[0]
    assert noise_result.page_content == noise_doc.page_content
    assert noise_result.metadata == noise_doc.metadata
...
def test_search_documents_01(
    ...
    """
    # 試験準備
    VaultDb.sync(vault_docs, dk_cxt.get_engine(), dk_entity)
    dk_obsidian_retriever.sync_chunks()
    doc = vault_docs[0]
    ...
    results_date = dk_obsidian_retriever.search_documents(
    ...
    results_path = dk_obsidian_retriever.search_documents("project", top_k=10, filters=path_filter)
