...
def test_get_documents_by_ids_01(
    ...
    results = obsidian_retriever.get_documents_by_ids([doc.id])
    ...
    # 観点2
    assert obsidian_retriever.get_documents_by_ids(["nonexistent-uuid"]) == []
...
def test_sync_chunks_01(
    ...
    # 観点1
    results = obsidian_retriever.get_documents_by_ids([noise_doc.id, target_doc.id])
    ...
    # 観点2
    target_result = obsidian_retriever.get_documents_by_ids([target_doc.id])[0]
    assert target_result.page_content == updated_target.page_content
    assert target_result.metadata == updated_target.metadata
    noise_result = obsidian_retriever.get_documents_by_ids([noise_doc.id])[0]
    assert noise_result.page_content == noise_doc.page_content
    assert noise_result.metadata == noise_doc.metadata
    ...
    # 観点3
    assert obsidian_retriever.get_documents_by_ids([target_doc.id]) == []
    noise_result = obsidian_retriever.get_documents_by_ids([noise_doc.id])[0]
    assert noise_result.page_content == noise_doc.page_content
    assert noise_result.metadata == noise_doc.metadata
