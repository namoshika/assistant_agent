...
async def test_initialize_01(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    ...
    mocker.patch("assistant_agent.services.vault_obsidian.PGEngine.from_engine")
    m_create = mocker.patch(
        "assistant_agent.services.vault_obsidian.PGVectorStore.create",
        new_callable=mocker.AsyncMock,
    )
    m_create.return_value.asimilarity_search = mocker.AsyncMock(return_value=[])
    ...
    await retriever.search_documents("テスト", top_k=1, filters=None)
    m_store_conn.get_engine.assert_called_once()
    m_create.assert_called_once()


async def test_search_documents_01(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    """クエリで Chunk 検索し、類似する Document を取得できるか確認 (filter 省略).

    返却 Document は id + metadata のみ保持し page_content は空。

    観点1: asimilarity_search が引数 k 付き、filter=None で呼ばれていること
    ...
    m_vector_store.asimilarity_search = mocker.AsyncMock(
        return_value=[chunk_a1, chunk_b1, chunk_a2]
    )

    # 試験実施
    result = await retriever.search_documents("テスト", top_k=5, filters=None)

    # 結果検証
    # 観点1
    m_vector_store.asimilarity_search.assert_called_once_with("テスト", k=5, filter=None)
    # 観点2 & 観点3
    assert result[0] == chunk_a1.model_copy(update={"page_content": ""})
    assert result[1] == chunk_b1.model_copy(update={"page_content": ""})


async def test_search_documents_02(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    """クエリで Chunk 検索し、類似する Document リストを取得できるか確認 (filter 有り).

    観点1: asimilarity_search が filter に変換後の dict 付きで呼ばれること
    """
    # 試験準備
    ...
    m_vector_store = m_create.return_value
    m_vector_store.asimilarity_search = mocker.AsyncMock(return_value=[])

    # 試験実施
    await retriever.search_documents("テスト", top_k=5, filters=filters)
...
async def test_search_documents_03(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    ...
    観点1: asimilarity_search() が空リストを返すとき [] を返す
    """
    # 試験準備
    mocker.patch.object(retriever, "_store_conn")
    mocker.patch("assistant_agent.services.vault_obsidian.PGEngine.from_engine")
    ...
    # 試験実施
    result = await retriever.search_documents("クエリ", top_k=5, filters=None)
...
async def test_get_documents_by_ids_01(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    ...
    m_session = MagicMock()
    m_session.__aenter__ = mocker.AsyncMock(return_value=m_session)
    m_session.__aexit__ = mocker.AsyncMock(return_value=False)
    m_scalar_result = MagicMock()
    m_session.scalars = mocker.AsyncMock(return_value=m_scalar_result)
    mocker.patch("assistant_agent.services.vault_obsidian.AsyncSession", return_value=m_session)

    # 観点1: DBが逆順 (b→a) で返しても document_ids の順 (a→b) で返る
    m_session.scalars.return_value.all.return_value = [m_row_b, m_row_a]
    docs = await retriever.get_documents_by_ids([_DOC_ID_A, _DOC_ID_B])
    assert len(docs) == 2
    ...
    docs = await retriever.get_documents_by_ids([_DOC_ID_A, unknown_id])
    assert len(docs) == 1
    assert docs[0].page_content == "content_a"

    # 観点3: 空リスト入力 → 空リストを返す
    m_session.scalars.return_value.all.return_value = []
    docs = await retriever.get_documents_by_ids([])
    assert docs == []


async def test_sync_chunks_01(retriever: VaultObsidianRetriever, mocker: MockerFixture):
    ...
    m_vector_store = m_create.return_value
    m_vector_store.aadd_documents = mocker.AsyncMock()
    mocker.patch(
    ...
    # 試験実施
    await retriever.sync_chunks()

    # 結果検証
    chunks = m_vector_store.aadd_documents.call_args.args[0]
    # 観点1
    assert len(chunks) == 2
...
async def test_sync_chunks_02(retriever: VaultObsidianRetriever, mocker: MockerFixture):
