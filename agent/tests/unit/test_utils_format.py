...
from assistant_agent.utils import format
from assistant_agent.utils.format import ContentsWithFrontmatter
...
def test_format_doc_ids_01():
    ...
    # 試験実施
    result = format.format_doc_ids(docs)
...
def test_format_doc_list_01():
    ...
    """
    # 試験準備
    doc_with_links = ContentsWithFrontmatter(
        id="doc-id-1",
        title="note.md",
        contents="ノート本文",
        frontmatter={
    ...
    doc_without_links = ContentsWithFrontmatter(
        id="doc-id-2",
        title="plain.md",
        contents="リンクなし本文",
    ...
    # 試験実施
    result_with = format.format_doc_list([doc_with_links])
    result_without = format.format_doc_list([doc_without_links])
    ...
    # 観点2
    assert "note.md" in result_with
    assert "ノート本文" in result_with
    assert "YAML 日本語テキスト" in result_with
    assert "folder/note.md" in result_with
    assert "linked-id-1" in result_with
    assert "linked-id-2" in result_with
    assert "linked1.md" in result_with
    assert "linked2.md" in result_with
    # 観点3
    assert "リンクなし本文" in result_without
